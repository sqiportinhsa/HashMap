...
* `ror hash`, `rol hash` - значение хеш-функции вычисляется в цикле по символам ключа, где на кажом шаге производятся операции циклического сдвига вправо и влево соответственно, и прибавление к хешу ASCII-кода символа.
...
Дополнительно, средняя длина цепочки меньше среднеквадратичного отклонения, что говорит о неравномерности.
...
Предварительный анализ:
> Функция способна покрывать весь диапазон возможных значений переменной типа `unsigned int`. Действительно, переменная данного типа содержит $32$ бита. Мы используем ключи, состоящие из английских букв, что значит, что изначально мы прибавим к переменной значение, занимающее $6$ бит, старший из них ненулевой. При этом на каждом шаге самый правый бит сдвигается на $1$ вправо, что значит, что за $26$ шагов мы гарантированно дойдем до старшего бита всей переменной. Однако, значения хеша сильно зависит от длины слова. Это происходит из-за того, что до момента заполнения старших битов циклический сдвиг вправо будет совпадать с обычным битовым сдвигом вправо. Таким образом с на каждой новой букве значение хеша увеличивается не менее, чем в два раза. Значит значения хешей будут группироваться вблизи средних значений для данной длины.
...
Вывод: хеш-функция неприменима.

#### `Rol hash`

Предварительный анализ:
> Функция способна покрывать все значения хешей в таблице. Действительно, уже при первых сдвигах мы начинаем занимать старшие биты переменной, содержащей хеш. При этом за счет суммирования младшие биты постоянно обновляются, и на последующих шагах обновляют старшие. Таким образом любой символ значительно влияет на значение хеша. Это позволяет предположить близкое к равномерному распределение, покрывающее весь диапазон возможных значений хеша.
