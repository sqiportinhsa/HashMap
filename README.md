...
Инлайн функции поиска в списке не даст какого-либо выигрыша из-за продолжительности работы вышестоящей функции и размера текущей. Таким образом мы без потери производительности можем вынести ее в отдельный файл для удобства работы. В качестве ассемблера будем использовать NASM. [Исходный код](src/list_find.asm).
